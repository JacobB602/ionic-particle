...
export class VariablePage {
  ...
  public Voltage: any;                     // Contains the value of our cloud variable
  ...
  ionViewDidEnter() {
    // When entering the page, subscribe to updates to the Particle cloud varibale Voltage
    if (this.particle.device) {
        this.cancelSubscription();
        this.subscription = this.particle.pollVariable("Voltage").subscribe(
            (value) => { this.Voltage = value; this.subscribed = true; },
            (error) => { console.log("Error reading Voltage"); this.subscribed = false; },
            () => { console.log("Stopped polling Voltage"); this.subscribed = false; }
        );
    }
  }
